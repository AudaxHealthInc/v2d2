...
import v2d2.protocols.CardResponse
...
class MagicSpec()
  ...
  "A MagicAct" must {
      ...
      val parent = system.actorOf(Props(new Actor {
        ...
        def receive = {
          case x if sender == child =>
            x match {
              case r: CardResponse =>
                assert(r.cards.length == 1)
                assert(r.cards.head.name == "Shivan Dragon")
              case _ => 
                pprint.log("fail")
                assert(false)
            }
          ...
          case x if sender == child =>
            x match {
              case r: CardResponse =>
                pprint.log(r)
                assert(r.cards.length == 1)
                assert(r.cards.head.name == "Goblin")
              case _ => assert(false)
            }
            proxy.ref.forward(x)
          case x => child.forward(x)
    ...
    //           case r: CardResponse =>
    //             assert(r.cards.length == 1)
    //             assert(r.cards.head.name == "Black Lotus")
    //           case _ => assert(false)
    //         }
    //         proxy.ref.forward(x)
    //       case x => child.forward(x)
    ...
    //             pprint.log(r)
    //             assert(r.cards.length != 1)
    //             assert(r.cards.head.name.contains("""Sun's Avatar"""))
    //           case _ => assert(false)
    //         }
    //         proxy.ref.forward(x)
    //       case x => child.forward(x)
